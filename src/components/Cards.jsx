  ...
  return (
    <div className="max-w-md  mx-auto my-8 p-4">
      ...
      style={{ backgroundImage: 'linear-gradient(135deg, #EDFC74, #F5FFA8)' }}
        className="relative overflow-hidden w-[311px] rounded-xl shadow-lg text-white p-5 h-[184px]"
      ...
      >
        {cards1.map((card, index) => (
          <div 
            key={card.id}
            className={`absolute inset-0 p-6 transition-opacity duration-500 ${index === activeIndex ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}
          >
      ...
      <div className="flex justify-center mt-4 space-x-2">
        {cards1.map((_, index) => (
          <button
            key={index}
            className={`w-2 h-2 rounded-full ${index === activeIndex ? 'bg-[#EDFC74] w-2.5 h-2.5 -translate-y-[2px]' : 'bg-gray-300'}`}
